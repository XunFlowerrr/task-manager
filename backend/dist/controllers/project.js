...
export async function createProject(req, res) {
    ...
    try {
        ...
        const sanitizedProjectDescription = projectDescription
        ...
        await query(`INSERT INTO project (project_id, project_name, project_description, owner_id, category)
       VALUES ($1, $2, $3, $4, $5)`, [
            projectId,
        ...
        await query(`INSERT INTO project_member (project_id, user_id) VALUES ($1, $2)`, [projectId, ownerId]);
        log.debug(`createProject: Owner ${ownerId} added to project_member for project ${projectId}`);
...
export async function getAllProjects(req, res) {
    ...
    try {
        const result = await query(`SELECT p.* FROM project p
       LEFT JOIN project_member pm ON p.project_id = pm.project_id
       WHERE p.owner_id = $1 OR pm.user_id = $1`, [req.user.userId]);
        log.debug("getAllProjects: Query result: " +
...
export async function getProject(req, res) {
    ...
    try {
        ...
        const result = await query(`SELECT p.* FROM project p
       LEFT JOIN project_member pm ON p.project_id = pm.project_id
       WHERE p.project_id = $1 AND (p.owner_id = $2 OR pm.user_id = $2)`, [id, req.user.userId]);
        log.debug("getProject: Query result: " +
            JSON.stringify({ rowCount: result.rowCount }));
...
export async function updateProject(req, res) {
    log.info("updateProject: Request received, id=" +
        req.params.id +
        ", body=" +
    ...
    try {
        ...
        await query(`UPDATE project SET project_name=$1, project_description=$2, category=$3
       WHERE project_id=$4`, [sanitizedProjectName, sanitizedProjectDescription, sanitizedCategory, id]);
