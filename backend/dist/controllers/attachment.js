...
async function checkProjectAccess(projectId, userId) {
    const accessCheck = await query(`SELECT p.project_id
     FROM project p
     LEFT JOIN project_member pm ON p.project_id = pm.project_id
     WHERE p.project_id = $1 AND (p.owner_id = $2 OR pm.user_id = $2)`, [projectId, userId]);
...
async function checkTaskAccess(taskId, userId) {
    const taskCheck = await query(`SELECT t.task_id, t.project_id
     FROM task t
     JOIN project p ON t.project_id = p.project_id
     LEFT JOIN project_member pm ON p.project_id = pm.project_id
     WHERE t.task_id = $1 AND (p.owner_id = $2 OR pm.user_id = $2)`, [taskId, userId]);
...
export async function uploadAttachment(req, res) {
    ...
    try {
        ...
        if (!taskAccess) {
            log.warn(`uploadAttachment: User ${userId} not authorized for task ${taskId}`);
        ...
        const result = await query(`INSERT INTO attachment (attachment_id, task_id, file_name, file_path, file_type, file_size)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`, [
            attachmentId,
            taskId,
        ...
        log.info(`uploadAttachment: Attachment ${attachmentId} uploaded for task ${taskId} by user ${userId}`);
        res.status(201).json({
        ...
        if (req.file) {
            try {
                ...
                log.debug(`uploadAttachment: Cleaned up file ${req.file.filename} after error.`);
            }
            catch (unlinkError) {
                log.error(`uploadAttachment: Failed to clean up file ${req.file.filename}: ${unlinkError}`);
            }
        }
...
export async function getAllAttachments(req, res) {
    ...
    try {
        ...
        if (taskId) {
         ...
         JOIN project p ON t.project_id = p.project_id
         LEFT JOIN project_member pm ON p.project_id = pm.project_id
         WHERE p.owner_id = $1 OR pm.user_id = $1`, [userId]);
...
export async function downloadAttachment(req, res) {
    ...
    try {
        ...
        const attachmentRes = await query(`SELECT a.*, t.project_id
       FROM attachment a
       JOIN task t ON a.task_id = t.task_id
       WHERE a.attachment_id = $1`, [id]);
        ...
        try {
            ...
            log.info(`downloadAttachment: Sending file ${attachment.file_path} for attachment ${id}`);
            ...
            res.download(filePath, attachment.file_name, (err) => {
                if (err) {
                    ...
                    log.error(`downloadAttachment: Error sending file ${attachment.file_path}: ${err}`);
            ...
            log.error(`downloadAttachment: File not found or inaccessible at ${filePath} for attachment ${id}: ${fileError}`);
...
export async function deleteAttachment(req, res) {
    ...
    try {
        ...
        const attachmentCheck = await query(`SELECT a.attachment_id, a.file_path, t.project_id
       FROM attachment a
       JOIN task t ON a.task_id = t.task_id
       JOIN project p ON t.project_id = p.project_id
       LEFT JOIN project_member pm ON p.project_id = pm.project_id
       WHERE a.attachment_id = $1 AND (p.owner_id = $2 OR pm.user_id = $2)`, [id, userId]);
        if (attachmentCheck.rowCount === 0) {
            ...
            const exists = await query("SELECT 1 FROM attachment WHERE attachment_id = $1", [id]);
        ...
        try {
            ...
            log.debug(`deleteAttachment: File ${attachment.file_path} deleted from filesystem`);
        }
        catch (unlinkError) {
            // Log error but proceed to delete DB record, maybe the file was already gone
            log.error(`deleteAttachment: Failed to delete file ${attachment.file_path}: ${unlinkError}. Proceeding with DB deletion.`);
        }
