...
export async function searchUsers(req, res) {
    ...
    log.info(`searchUsers: Request received, query='${q}', projectId='${projectId}', requestingUser='${requestingUserId}'`);
    ...
    try {
        ...
        const accessCheckRes = await query(`SELECT 1 FROM project_member
       WHERE project_id = $1 AND user_id = $2
       UNION
       SELECT 1 FROM project
       WHERE project_id = $1 AND owner_id = $2`, [projectId, requestingUserId]);
        if (accessCheckRes.rowCount === 0) {
            log.warn(`searchUsers: User ${requestingUserId} not authorized for project ${projectId}`);
        ...
        const result = await query(`SELECT user_id, username, email
       FROM users
       ...
       LIMIT 10`, // Limit results for performance
        ...
        log.info(`searchUsers: Found ${result.rowCount} users matching query '${q}' for project '${projectId}'`);
