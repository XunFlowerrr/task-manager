...
export async function register(req, res) {
    try {
        ...
        // Create the user
        await query(`INSERT INTO users (user_id, username, email, password, role)
       VALUES ($1, $2, $3, $4, $5)`, [user_id, sanitizedUsername, sanitizedEmail, hashedPassword, role]);
...
export async function getCurrentUser(req, res) {
    try {
        ...
        const userRes = await query("SELECT user_id, username, email, role FROM users WHERE user_id = $1", [userId]);
        if (userRes.rowCount === 0) {
...
function generateToken(user) {
    return jwt.sign({
        ...
        email: user.email,
        role: user.role,
    }, config.jwtSecret, { expiresIn: "15d" });
}
