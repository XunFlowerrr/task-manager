...
const log = logger("tasks.ts");
export async function createTask(req, res) {
    log.info("createTask: Request received, body=" + JSON.stringify(req.body));
    try {
        const { projectId, taskName, taskDescription, startDate, dueDate, status, priority, assignees, } = req.body;
        ...
        const membershipCheck = await query(`SELECT p.project_id
       FROM project p
       LEFT JOIN project_member pm ON p.project_id = pm.project_id
       WHERE p.project_id = $1 AND (p.owner_id = $2 OR pm.user_id = $2)`, [sanitizedProjectId, req.user.userId]);
        ...
        await query(`INSERT INTO task (task_id, project_id, task_name, task_description, start_date, due_date, status, priority)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [
        ...
        if (assignees && Array.isArray(assignees) && assignees.length > 0) {
            ...
            for (const assigneeId of assignees) {
                const isMember = await query(`SELECT user_id FROM project_member
           WHERE project_id = $1 AND user_id = $2
           UNION
           SELECT owner_id FROM project
           WHERE project_id = $1 AND owner_id = $2`, [sanitizedProjectId, assigneeId]);
                if (isMember.rowCount && isMember.rowCount > 0) {
                    await query("INSERT INTO task_assignee (task_id, user_id) VALUES ($1, $2)", [taskId, assigneeId]);
                    log.debug(`createTask: User ${assigneeId} assigned to task ${taskId}`);
                }
                else {
                    log.warn(`createTask: Attempted to assign non-member ${assigneeId} to task ${taskId}`);
                }
            }
        }
...
export async function getAllTasks(req, res) {
    ...
    try {
       ...
       FROM project p
       LEFT JOIN project_member pm ON p.project_id = pm.project_id
       WHERE p.project_id = $1 AND (p.owner_id = $2 OR pm.user_id = $2)`, [projectId, req.user.userId]);
        if (membershipCheck.rowCount === 0) {
            res.status(403).json({ error: "Not authorized" });
            return;
        }
...
export async function getTask(req, res) {
    ...
    try {
       ...
       LEFT JOIN project_member pm ON p.project_id = pm.project_id
       WHERE t.task_id = $1 AND (p.owner_id = $2 OR pm.user_id = $2)`, [id, req.user.userId]);
...
export async function updateTask(req, res) {
    log.info("updateTask: Request received, params=" +
        JSON.stringify(req.params) +
        ", body=" +
        JSON.stringify(req.body));
    try {
        const { id } = req.params;
        const { taskName, taskDescription, startDate, dueDate, status, priority, assignees, } = req.body;
        if (!taskName) {
            res.status(400).json({ error: "Missing required fields." });
            return;
        }
        ...
        const membershipCheck = await query(`SELECT t.*, p.project_id
       FROM task t
       JOIN project p ON t.project_id = p.project_id
       LEFT JOIN project_member pm ON p.project_id = pm.project_id
       WHERE t.task_id = $1 AND (p.owner_id = $2 OR pm.user_id = $2)`, [id, req.user.userId]);
        ...
        // Update task details
        await query(`UPDATE task SET task_name=$1, task_description=$2, start_date=$3, due_date=$4, status=$5, priority=$6
       WHERE task_id=$7`, [
            sanitizedTaskName,
            sanitizedTaskDescription,
            startDate,
        ...
        if (assignees && Array.isArray(assignees)) {
            ...
            if (assignees.length > 0) {
             ...
             UNION
             SELECT owner_id FROM project
             WHERE project_id = $1 AND owner_id = $2`, [projectId, assigneeId]);
                    if (isMember.rowCount && isMember.rowCount > 0) {
                        await query("INSERT INTO task_assignee (task_id, user_id) VALUES ($1, $2)", [id, assigneeId]);
                        log.debug(`updateTask: User ${assigneeId} assigned to task ${id}`);
                    }
                    else {
                        log.warn(`updateTask: Attempted to assign non-member ${assigneeId} to task ${id}`);
                    }
                }
            }
        }
...
export async function deleteTask(req, res) {
    ...
    try {
        const { id } = req.params;
        const membershipCheck = await query(`SELECT t.*
       FROM task t
       JOIN project p ON t.project_id = p.project_id
       LEFT JOIN project_member pm ON p.project_id = pm.project_id
       WHERE t.task_id = $1 AND (p.owner_id = $2 OR pm.user_id = $2)`, [id, req.user.userId]);
...
// Functions for managing task assignees
export async function assignUser(req, res) {
    log.info("assignUser: Request received, params=" +
        JSON.stringify(req.params) +
        ", body=" +
        JSON.stringify(req.body));
    try {
        ...
        const taskCheck = await query(`SELECT t.task_id, p.project_id
       FROM task t
       JOIN project p ON t.project_id = p.project_id
       LEFT JOIN project_member pm ON p.project_id = pm.project_id
       WHERE t.task_id = $1 AND (p.owner_id = $2 OR pm.user_id = $2)`, [taskId, req.user.userId]);
        ...
        const memberCheck = await query(`SELECT user_id FROM project_member
       WHERE project_id = $1 AND user_id = $2
       UNION
       SELECT owner_id FROM project
       WHERE project_id = $1 AND owner_id = $2`, [projectId, userId]);
        if (memberCheck.rowCount === 0) {
            res.status(400).json({ error: "User is not a member of the project" });
            return;
        }
        // Check if the assignment already exists
        const assignmentCheck = await query("SELECT * FROM task_assignee WHERE task_id = $1 AND user_id = $2", [taskId, userId]);
        if (assignmentCheck.rowCount && assignmentCheck.rowCount > 0) {
            res.status(400).json({ error: "User is already assigned to this task" });
            return;
        }
        // Add assignment
        await query("INSERT INTO task_assignee (task_id, user_id) VALUES ($1, $2)", [taskId, userId]);
...
export async function unassignUser(req, res) {
    log.info("unassignUser: Request received, params=" + JSON.stringify(req.params));
    try {
        const { taskId, userId } = req.params;
        // Check if user has permission to modify the task
        const taskCheck = await query(`SELECT t.task_id
       FROM task t
       JOIN project p ON t.project_id = p.project_id
       LEFT JOIN project_member pm ON p.project_id = pm.project_id
       WHERE t.task_id = $1 AND (p.owner_id = $2 OR pm.user_id = $2)`, [taskId, req.user.userId]);
        ...
        // Remove assignment
        await query("DELETE FROM task_assignee WHERE task_id = $1 AND user_id = $2", [taskId, userId]);
...
export async function getTaskAssignees(req, res) {
    log.info("getTaskAssignees: Request received, params=" + JSON.stringify(req.params));
    try {
        const { taskId } = req.params;
       ...
       WHERE t.task_id = $1 AND (p.owner_id = $2 OR pm.user_id = $2)`, [taskId, req.user.userId]);
        ...
        const assignees = await query(`SELECT ta.user_id, u.username, u.email
       FROM task_assignee ta
       JOIN users u ON ta.user_id = u.user_id
       WHERE ta.task_id = $1`, [taskId]);
        log.info(`getTaskAssignees: Retrieved ${assignees.rowCount} assignees for task ${taskId}`);
...
export async function getUserTasks(req, res) {
    ...
    try {
        ...
        const tasks = await query(`SELECT t.*, p.project_name,
        (SELECT json_agg(json_build_object('user_id', u.user_id, 'username', u.username, 'email', u.email))
         FROM task_assignee ta
         JOIN users u ON ta.user_id = u.user_id
         WHERE ta.task_id = t.task_id) as assignees
       ...
       JOIN project p ON t.project_id = p.project_id
       WHERE ta.user_id = $1
       ORDER BY t.due_date ASC NULLS LAST, t.priority DESC`, [userId]);
        log.info(`getUserTasks: Retrieved ${tasks.rowCount} tasks for user ${userId}`);
