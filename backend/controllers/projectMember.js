...
export async function getProjectMembers(req, res) {
  log.info(
    "getProjectMembers: Request received, params=" + JSON.stringify(req.params)
  );
  try {
    const { projectId } = req.params;
    ...
    if (accessCheckRes.rowCount === 0) {
      log.warn(
        `getProjectMembers: User ${requestingUserId} not authorized for project ${projectId}`
      );
      return res
        .status(403)
        .json({ error: "Not authorized to view members of this project" });
    }

    // 2. Fetch members and their details
    const membersRes = await query(
      `SELECT pm.user_id, pm.project_id, u.username, u.email
    ...
    log.info(
      `getProjectMembers: Found ${membersRes.rowCount} members for project ${projectId}`
    );
