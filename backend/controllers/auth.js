...
export async function register(req, res) {
    log.info("register: Request received, body=" + JSON.stringify({ username: req.body.username, email: req.body.email }));
    try {
        ...
        const existing = await query('SELECT * FROM users WHERE email = $1', [sanitizedEmail]);
        ...
        log.info(`register: User ${user_id} registered with email ${sanitizedEmail}`);
...
export async function login(req, res) {
    ...
    try {
        ...
        const sanitizedEmail = email.trim();
        // Look up the user
        const userRes = await query('SELECT * FROM users WHERE email = $1', [sanitizedEmail]);
...
function sendTokenResponse(user,statusCode, res) {
    ...
    res.status(statusCode).cookie('token', token, options).json({ message: 'Token generated successfully' });
